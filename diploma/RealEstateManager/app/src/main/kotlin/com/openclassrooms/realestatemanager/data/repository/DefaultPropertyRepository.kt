...
@Inject constructor(
            ...
            .flatMap { isInternetAvailable ->
                if (isInternetAvailable) {
                    Observable
                        ...
                        .filter {
                            cachedProperties.all { property -> !property.locallyUpdated } &&
    ...
    override fun findProperty(propertyId: String): Observable<Property> {
        return findCompleteProperty(propertyId).startWith(
            cacheDataSource.findById(
                Property::class,
                propertyId
            ).toObservable()
        )
    ...
    private fun findCompleteProperty(propertyId: String): Observable<Property> {
        return if (networkConnectionLiveData.value == true) {
            Observable.zip(
                remoteDataSource.findById(Property::class, propertyId).toObservable(),
    ...
    override fun updateProperty(updatedProperty: Property): Observable<Boolean> {
        return Observable.just(networkConnectionLiveData.value).flatMapCompletable { isInternetAvailable ->
            if (isInternetAvailable) {
                updateRemotelyProperty(updatedProperty = updatedProperty)
            } else {
                updateLocallyProperty(updatedProperty = updatedProperty)
            }
        }.andThen(
            ObservableSource { observer ->
                cachedProperties.single { property -> property.id == updatedProperty.id }
                    .let { property ->
                        cachedProperties[cachedProperties.indexOf(property)] = updatedProperty
                    }
            ...
            .andThen(
                ...
                if (updatedProperty.photos.any { photo -> photo.locallyDeleted }) {
                    remoteDataSource.delete(
                        Photo::class,
                        updatedProperty.photos.filter { photo -> photo.locallyDeleted }
                    )
                } else { complete() }
            )
        ...
        return cacheDataSource.update(Property::class, updatedProperty.apply { locallyUpdated = true })
            .andThen(
                if (updatedProperty.photos.any { photo -> photo.locallyUpdated }) {
                    cacheDataSource.update(
                        Photo::class,
                        updatedProperty.photos.filter { photo -> photo.locallyUpdated }
                    )
                } else { complete() }
            ).andThen(
                if (updatedProperty.photos.any { photo -> photo.locallyCreated }) {
    ...
    override fun createProperty(createdProperty: Property): Observable<Boolean> {
        return Observable.just(networkConnectionLiveData.value).flatMapCompletable { isInternetAvailable ->
            if (isInternetAvailable) {
                createRemotelyProperty(createdProperty = createdProperty)
            } else {
                createLocallyProperty(createdProperty = createdProperty)
            }
        }.andThen(
            ObservableSource { observer ->
        ...
        return remoteDataSource.save(Property::class, createdProperty)
            ...
            .andThen(
                if (createdProperty.photos.isNotEmpty()) {
                    cacheDataSource.save(
                        Photo::class,
                        createdProperty.photos.onEach { photo ->
                            photo.locallyCreated = false
    ...
    fun updateProperties(): Observable<Pair<CreateOrUpdate, List<Property>>> {
        return Observable.just(
            cachedProperties.filter { property -> property.locallyUpdated && !property.locallyCreated }
        ).flatMap { properties ->
            if (properties.isNotEmpty()) {
                remoteDataSource.update(Property::class, properties)
                    .andThen(
                        if (properties.any { property ->
                            property.photos.any { photo -> photo.locallyUpdated && !property.locallyCreated }
                        }
                        ) {
                            val updatedPhotos = properties
                                .filter {
                                    ...
                                    property.photos.any {
                                        photo ->
                                        photo.locallyUpdated && !property.locallyCreated
                                    }
                                }
                                .map { property ->
                    ...
                    .andThen(
                        cacheDataSource.update(
                            Property::class,
                            properties.onEach { property -> property.locallyUpdated = false }
                        )
                    )
                    .andThen(
                        if (properties.any { property ->
                            ...
                            cacheDataSource.update(
                                Photo::class,
                                updatedPhotos.onEach { photo -> photo.locallyUpdated = false }
                            )
                        } else { complete() }
                    ).andThen(
                        if (properties.any { property -> property.photos.any { photo -> photo.locallyCreated } }) {
    ...
    private fun createProperties(): Observable<Pair<CreateOrUpdate, List<Property>>> {
        return Observable.just(
            cachedProperties.filter { property -> property.locallyCreated }
        ).flatMap { properties ->
            if (properties.isNotEmpty()) {
