        ...
        do {
            ...
            case ArithmeticError.CannotAddOperator:
                presentUIAlertController(title: error.rawValue.title,
                                         message: String(format: error.rawValue.message, textView.text  + "\(arithmeticOperator)"))
            default:
                break
            }
        } catch {}
    ...
    @IBAction private func tappedEqualButton(_ sender: UIButton) {
        do {
            try textView.text.append(" = \(formatter.string(from: Double(expression.evaluate())! as NSNumber) ?? String(expression.evaluate())) ")
             ...
             presentUIAlertController(title: error.rawValue.title, message: String(format: error.rawValue.message, textView.text))
        } catch {}
    }
