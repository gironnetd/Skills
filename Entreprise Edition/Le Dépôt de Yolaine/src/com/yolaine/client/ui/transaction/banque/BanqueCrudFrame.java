...
public class BanqueCrudFrame extends YolaineCrudFrame<BanquePane> {

	private static final long serialVersionUID = 7769342743144999626L;
	private int defaultWidth = 640;
	private int defaultHeight = 480;

	public BanqueCrudFrame(final BanquePane mainPane) {
		super(mainPane);

		mainPane.getModel().addXSListener(new BanqueAdapter() {

			@Override
			public void banqueChanged(
					XSEvent<BanqueEventPropertyName, String> evt) {
	...
	public void findActionPerformed(EventObject evt) {
		...
		BanqueModel model = mainPane.getModel();
		...
		try {
			Banque banque = CatalogDelegate.trouverBanque(model.getBanque().getId());
