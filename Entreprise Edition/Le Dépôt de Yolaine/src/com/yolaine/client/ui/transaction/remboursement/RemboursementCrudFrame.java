...
public class RemboursementCrudFrame extends YolaineCrudFrame<RemboursementPane> {

	private static final long serialVersionUID = 7769342743144999626L;
	private int defaultWidth = 640;
	private int defaultHeight = 480;

	public RemboursementCrudFrame(final RemboursementPane mainPane) {
		super(mainPane);

		mainPane.getModel().addXSListener(new RemboursementAdapter() {

			@Override
			public void dateremboursementChanged(
					XSEvent<RemboursementEventPropertyName, String> evt) {
	...
	public void findActionPerformed(EventObject evt) {
		...
		RemboursementModel model = mainPane.getModel();


		if (model.getBanqueRemboursement().getBanque() == null || model.getBanqueRemboursement().getBanque().equals("")) {
		...
		try {
			Banque banque = CatalogDelegate.trouverBanque(model.getBanqueRemboursement().getId());
			...
			if (banque == null) {
				...
				model.setBanqueRemboursement(banque);
	...
	public void createActionPerformed(EventObject evt) {
		...
		RemboursementModel model = mainPane.getModel();
		Banque banque = model.getBanqueRemboursement();
	...
	public void deleteActionPerformed(EventObject evt) {
		...
		for(Article article: listearticles){
		 ...
		 if (listeBanques.size() == 1){
			...
			JOptionPane.showMessageDialog(this,
					"Ce type de manche ne peut pas être supprimé car \n il précise \n le type de manches d' un article.", "Impossible",
					JOptionPane.WARNING_MESSAGE);
		}else  if (listeBanques.size() > 1){
			
			JOptionPane.showMessageDialog(this,
					"Ce type de manche ne peut pas être supprimé car \n il précise \n le type de manches de plusieurs articles.", "Impossible",
					JOptionPane.WARNING_MESSAGE);
		}else{
